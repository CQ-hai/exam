...
public interface JudgeQuestionMapper extends BaseMapper<JudgeQuestion> {
    ...
    @Insert("insert into judge_question(subject,question,answer,analysis,level,section,bank_id,bank_name) values " +
    ...
    @Select("select questionId from judge_question  where bank_id=#{bankId}  order by rand() desc limit #{pageNo}")
